...
export default function Home() {
  ...
  const handleFilters = (key: keyof RestaurantFilters, value: string | null) => {
    setFilters((prev) => ({ ...prev, [key]: value }));
  };

  const { restaurants, isFetchingNextPage } = useGetRestaurantsQuery(filters);
        ...
        <div>
          ...
          <div className="manual_grid_300 -mx-4 mt-8 md:-mx-2 md:mt-5">
            {isFetchingNextPage ? (
              ...
              restaurantItems.map((item) => <RestaurantItem key={item.id} item={item} />)
            )}
          </div>
        </div>
      </div>
